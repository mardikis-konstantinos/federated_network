...
export default function ManageUsersPage() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_LOCAL_BACKEND_URL}users/by-role/user`
      )
  ...
  const handleCreate = async () => {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_LOCAL_BACKEND_URL}users/doctor`,
        {
          method: "POST",
  ...
  const handleUpdate = async () => {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_LOCAL_BACKEND_URL}users/${id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(`${import.meta.env.VITE_LOCAL_BACKEND_URL}users/${id}`, {
        method: "DELETE",
      })
      fetchUsers()
    } catch (err) {
  ...
  const handleViewLogs = async (id) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_LOCAL_BACKEND_URL}users/${id}/events?max=50`
      )
      const data = await res.json()
      setLogs(data.events)
      setLogsOpen(true)
